...
export function AdminBoardManager() {
  ...
  return <div className="space-y-6"><div className="flex items-center justify-between"><div className="space-y-1"><h3 className="text-xl font-semibold flex items-center gap-2"><Users className="h-5 w-5"/>Manage Board</h3><p className="text-sm text-muted-foreground">Add and edit board members</p></div><Button size="sm" className="flex items-center gap-2" onClick={addOptional}><Plus className="h-4 w-4"/>Add Member</Button></div>{showForm && editing && <Card><CardHeader><CardTitle className="text-sm font-semibold flex items-center gap-2">{editing.id? 'Edit':'Add'} Board Member</CardTitle></CardHeader><CardContent><form onSubmit={saveMember} className="space-y-4"><div className="grid md:grid-cols-2 gap-4"><div className="space-y-2"><Label>Name</Label><Input value={editing.name} onChange={e=>setEditing(m=> m? { ...m, name:e.target.value }: m)} required /></div><div className="space-y-2"><Label>Role</Label><Input value={editing.role} onChange={e=>setEditing(m=> m? { ...m, role:e.target.value }: m)} disabled={REQUIRED_ROLES.includes(editing.role)} required /></div><div className="space-y-2"><Label>Email</Label><Input type="email" value={editing.email} onChange={e=>setEditing(m=> m? { ...m, email:e.target.value }: m)} /></div><div className="space-y-2"><Label>Phone / Contact</Label><Input value={editing.phone} onChange={e=>setEditing(m=> m? { ...m, phone:e.target.value }: m)} /></div><div className="space-y-2"><Label>LinkedIn URL</Label><Input value={editing.linkedin || ''} onChange={e=>setEditing(m=> m? { ...m, linkedin:e.target.value }: m)} /></div><div className="space-y-2 md:col-span-2"><Label>Photo</Label><ImageDropzone existingUrl={editing.imageUrl || null} previousPath={editing.imagePath || null} pathPrefix={`board_members/${editing.role || 'member'}-`} onUploaded={({url, path})=> setEditing(m=> m? { ...m, imageUrl:url, imagePath:path }: m)} description="Drag & drop or click to select." /></div></div><div className="flex gap-2"><Button type="submit" size="sm" className="gap-1"><Save className="h-4 w-4"/>{editing.id? 'Update':'Save'}</Button><Button type="button" size="sm" variant="outline" onClick={reset}>Cancel</Button>{editing.imageUrl && <Button type="button" size="sm" variant="ghost" onClick={()=> setEditing(m=> m? { ...m, imageUrl:'', imagePath:'' }: m)}>Remove Photo</Button>}</div></form></CardContent></Card>}<Card><CardHeader><CardTitle className="text-sm font-semibold">Board Members</CardTitle><CardDescription>{members.length} total (required roles cannot be deleted)</CardDescription></CardHeader><CardContent>{loading ? <div className="space-y-3">{[...Array(3)].map((_,i)=><Skeleton key={i} className="h-8 w-full"/> )}</div> : members.length===0 ? <p className="text-xs text-muted-foreground">No members yet.</p> : <div className="space-y-3">{members.map(m=> <div key={m.id || m.role} className="p-3 border rounded-md bg-muted/40 flex items-center gap-3"><div className="flex items-center gap-3 flex-1">{m.imageUrl ? <img src={m.imageUrl} className="h-12 w-12 rounded-full object-cover border" alt={m.name || m.role} /> : <div className="h-12 w-12 rounded-full bg-muted flex items-center justify-center text-[10px] text-muted-foreground">IMG</div>}<div className="space-y-1 min-w-0"><p className="text-sm font-medium truncate">{m.name || <span className="text-muted-foreground">(no name)</span>}</p><p className="text-[11px] text-muted-foreground truncate">{m.role.replace('_',' ')}</p>{(m.email || m.phone) && <p className="text-[10px] text-muted-foreground truncate">{m.email} {m.phone && ' • '+m.phone}</p>}{m.linkedin && <p className="text-[10px] text-muted-foreground truncate"><a href={m.linkedin} target="_blank" rel="noopener noreferrer">LinkedIn</a></p>}</div></div><div className="flex items-center gap-1"><Button size="icon" variant="ghost" className="h-7 w-7" onClick={()=>startEdit(m)}><Edit className="h-4 w-4"/></Button><Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" disabled={REQUIRED_ROLES.includes(m.role)} onClick={()=>handleDelete(m)}><Trash2 className="h-4 w-4"/></Button></div></div>)}</div>}</CardContent></Card></div>;
}
