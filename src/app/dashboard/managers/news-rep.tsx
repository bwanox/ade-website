...
export function ClubRepNewsManager({ clubId }: { clubId: string }) {
  ...
  const handleSubmit = async (e:React.FormEvent) => { e.preventDefault(); const base:any = { headline, summary, content, category, image, clubId, published:false, featured:false }; if (editingId) await updateWithTimestamp('news', editingId, base); else await createWithTimestamps('news', base); reset(); setShowForm(false); fetchArticles(); };
  const handleEdit = (a:NewsArticleDoc) => { setEditingId(a.id); setHeadline(a.headline||''); setSummary(a.summary||''); setContent(a.content||''); setCategory(a.category||''); setImage(a.image||''); setShowForm(true); };
