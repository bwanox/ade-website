...
export function AiNews({ enableRealtime = true, featuredOnly = false, limitCount = 6, retryAttempts = 3 }: AiNewsProps) {
  ...
  const parseArticle = (raw: any) => {
    if (raw.publishedAt?.toDate) {
      raw.date = raw.publishedAt.toDate().toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
    }
    const parsed = newsArticleSchema.safeParse(raw);
    if (!parsed.success) return null;
    return { ...(parsed.data as NewsArticleDoc), date: raw.date || '' };
  };

  const { data: articles, loading, error, reload } = useCollectionData<(NewsArticleDoc & { date?: string })>({
    query: buildQuery,
    enableRealtime,
    retryAttempts,
    parser: parseArticle,
  ...
  const trendingCount = useMemo(() => articles.filter(a => a.trending).length, [articles]);
  ...
  return (
    <section id="news" className="w-full py-20 relative overflow-hidden" aria-busy={loading} aria-live="polite">
      ...
      <div className="relative z-10 mb-16 text-center">
        ...
        {error && (
          <div className="mt-4 flex flex-col items-center gap-3" role="alert">
            ...
            <Button variant="outline" size="sm" onClick={() => { setAttempt(a => a + 1); reload(); }} disabled={loading} aria-label="Retry loading news">
              Retry
            </Button>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
        {(loading ? skeletonItems : articles).map((raw: any, index: number) => {
          const isSkeleton = loading;
          const article = isSkeleton ? null : raw;
          return (
            <Card
              key={isSkeleton ? index : article!.id}
              className={`overflow-hidden group/card flex flex-col cursor-pointer transform transition-all duration-500 ${isSkeleton ? 'opacity-70' : 'hover:scale-105 hover:-translate-y-2'} relative animate-fade-in-up`}
            ...
            >
              ...
              <CardContent className="p-6 flex-grow relative z-10">
                <h3 className="font-headline text-lg font-semibold mb-3 group-hover/card:text-accent transition-colors duration-300 relative leading-tight">
                  {isSkeleton ? <Skeleton className="h-5 w-3/4" /> : article!.headline}
                  {!isSkeleton && <div className="absolute -bottom-1 left-0 h-0.5 w-0 bg-gradient-to-r from-accent to-accent/50 group-hover/card:w-full transition-all duration-500" />}
                </h3>
                <p className="text-muted-foreground text-sm leading-relaxed mb-4 group-hover/card:text-foreground/80 transition-colors duration-300">
                  {isSkeleton ? <Skeleton className="h-4 w-full mb-2" /> : article!.summary}
                </p>
